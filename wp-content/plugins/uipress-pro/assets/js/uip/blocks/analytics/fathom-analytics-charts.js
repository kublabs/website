      ...
      processResponse() {
        ...
        if (dataType == 'avg_duration') {
          if (total !== 0 && totalValues > 0) {
            ...
            self.total = new Date(total * 1000).toISOString().substring(14, 19);
          }
          if (comparisonTotal !== 0 && totalCompValues > 0) {
            comparisonTotal = (comparisonTotal / totalCompValues).toFixed(2);
            self.comparisonTotal = new Date(comparisonTotal * 1000).toISOString().substring(14, 19);
          }
        }
        ...
        if (dataType == 'bounce_rate') {
          ...
          self.comparisonTotal = self.comparisonTotal + '%';
        }

        if (total == 0 || comparisonTotal == 0) {
          self.percentChange = (total * 100).toFixed(0);
        } else {
          self.percentChange = ((total / comparisonTotal) * 100).toFixed(0);
        }
