...
import * as connectFathom from './fathom-connect-account.min.js';
...
export function moduleData() {
  return {
    components: {
      'uip-connect-fathom': connectFathom.moduleData(),
    },
    props: {
      display: String,
      name: String,
    ...
    watch: {
      ...
      'uiTemplate.fathomAnalytics.ready': {
        handler(newValue, oldValue) {
          this.getAnalytics();
        },
        deep: true,
    ...
    computed: {
      ...
      inlineAccountSwitch() {
        let option = this.uipress.get_block_option(this.block, 'block', 'inlineAccountSwitch');
    ...
    methods: {
      getAnalytics() {
        ...
        self.error = false;
        self.errorMessage = '';
        self.showFathomConnection = false;

        //Api is not ready yet. We will catch with attached watch
        if (!self.uipress.isObject(self.uiTemplate.fathomAnalytics)) {
          self.apiLoaded = false;
          return;
        }
        if (!('ready' in self.uiTemplate.fathomAnalytics)) {
          self.apiLoaded = false;
          return;
        }
        if (!self.uiTemplate.fathomAnalytics.ready) {
          self.apiLoaded = false;
          return;
        }
        self.apiLoaded = true;
        ...
        //Send request to API
        self.uiTemplate.fathomAnalytics.api('get', startDate, endDate).then((response) => {
          ...
          if (response.error) {
            ...
            if (response.type == 'no_account') {
              self.showFathomConnection = true;
            }
            return;
          }
      ...
      removeAnalyticsAccount() {
        let self = this;
        self.uiTemplate.fathomAnalytics.ready = false;
        //Send request to API
        self.uiTemplate.fathomAnalytics.api('removeAccount').then((response) => {
          //The API call was successful

          self.uiTemplate.fathomAnalytics.api('refresh').then((response) => {
            //The API call was successful
            self.uiTemplate.fathomAnalytics.ready = true;
            self.getAnalytics();
          });
        });
      },
    ...
    template: `
      ...
      <div class="uip-margin-bottom-xxs" v-if="showFathomConnection"><uip-connect-fathom :success="refreshAnalytics"></uip-connect-fathom></div>
      ...
      :class="{'uip-dark-mode' : darkMapTheme || uipData.userPrefs.darkTheme}">
        <div id='uip-fathom-map' class="uip-fathom-map uip-w-300 uip-h-200"></div>
