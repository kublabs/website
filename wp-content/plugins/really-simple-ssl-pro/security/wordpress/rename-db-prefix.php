...
function rsssl_rename_db_prefix() {
    ...
    // Verify DB copy. If failed, disalbe the option and exit
    if ( rsssl_verify_database_copy($new_prefix) !== true ){
	    ...
	    update_option('rsssl_db_prefix_rename_failed', true);
	    rsssl_update_option( 'rename_db_prefix', false );
	    return false;
    }
    ...
    if ( is_writable( $wpconfig_path ) ) {
		...
		//in case it failed before, remove the option
	    delete_option('rsssl_db_prefix_rename_failed');
